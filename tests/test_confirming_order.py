...
class TestAboutRent:
    @allure.description('Тест открытия Окна подтверждения заказа после заполнения формы "Про аренду"')
    ...
    def test_about_rent(self, driver, name, surname, address, metro, telephone, when, period, colour, comment):
        ...
        confirming_order = ConfirmingOrder(driver)
        confirming_order.click_button_view_status()
        confirming_order.wait_and_find_element(ConfirmingOrderLocator.BUTTON_ORDER_STATUS)

        assert BasePage(driver).is_displayed(ConfirmingOrderLocator.BUTTON_ORDER_STATUS)
